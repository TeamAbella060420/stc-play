...
const ConnectAnytimeAnywhere = (props: sharedProps) => {
  ...
  const animatedImageStyle = useAnimatedStyle(() => {
    ...
    const outputRange = [0, 0, -400, 0];
    const translateX = interpolate(sharedValue.value, inputRange, outputRange);
    return { transform: [{ translateX: withTiming(translateX, { duration: 400, easing: Easing.ease }) }, { scale: 0.65 }] };
  });

  const animatedContainerStyle = useAnimatedStyle(() => {
    const inputRange = [-3, -2, -1];
    const outputRange = [windowWidth * 2 + 400, 0, -windowWidth - 400];
    const translateX = interpolate(sharedValue.value, inputRange, outputRange);
