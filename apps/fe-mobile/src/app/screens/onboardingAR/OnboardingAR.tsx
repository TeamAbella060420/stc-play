...
const OnboardingAR = () => {
  ...
  const playAnimation = useCallback((toValue: number, duration: number = 800) => {
    if (!isAnimating.value) {
      ...
      animatedCarouselValue.value = withTiming(toValue, { duration, easing: Easing.ease }, () => {
  ...
  const onGestureEvent = useAnimatedGestureHandler({
    onEnd: event => {
      if (!isAnimating.value) {
        ...
        animatedCarouselValue.value = withTiming(toValue, { duration: 800, easing: Easing.ease }, () => {
          isAnimating.value = false;
