...
const NeverMissChanceToShine = (props: sharedProps) => {
  ...
  const animatedImageStyle = useAnimatedStyle(() => {
    ...
    const outputRange = [0, 0, windowWidth, 100];
    const translateX = interpolate(sharedValue.value, inputRange, outputRange);
    return { transform: [{ translateX: withTiming(translateX, { duration: 100 }) }, { translateY: -windowHeight * 0.2 }] };
  });

  const animatedContainerStyle = useAnimatedStyle(() => {
    const inputRange = [2, 3];
    const outputRange = [windowWidth * 2 + 400, 0];
    const translateX = interpolate(sharedValue.value, inputRange, outputRange);
