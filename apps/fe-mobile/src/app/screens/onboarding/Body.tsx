  ...
  return (
    <View style={[styles.bodyContainer, isRTL ? styles.bodyContainerRTL : styles.bodyContainerLTR]}>
      <View style={{ flexDirection: 'row' }}>
        {data.map((_, index) => (
          <DotsIndicator key={`dots_idx${index}`} sharedValue={sharedValue} activeIndex={index} />
        ))}
      </View>
      <Animated.Text style={[styles.bodyTitle, textAlign, textAnimationStyle]}>{data[activeImageIndex]?.title}</Animated.Text>
      <Animated.Text style={[styles.bodyDescription, textAlign, textAnimationStyle]}>{data[activeImageIndex]?.description}</Animated.Text>
    </View>
  );
};
